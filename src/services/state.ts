...
class AppState {
   ...
   */
  selectItem(itemId: string, additive = false): void {
    if (additive) {
      // Toggle the item in the selection
      const ids = this.editor.selectedItemIds.includes(itemId)
        ? this.editor.selectedItemIds.filter(id => id !== itemId)
        : [...this.editor.selectedItemIds, itemId];
      this.updateEditor({
        selectedItemIds: ids,
  ...
  addToSelection(itemIds: string[]): void {
    const newIds = [...this.editor.selectedItemIds];
  ...
  getSelectedItems(): PageItem[] {
    ...
    for (const id of this.editor.selectedItemIds) {
  ...
  pasteFromClipboard(): PageItem[] {
    ...
    for (const item of this.editor.clipboard) {
  ...
  deleteSelectedItems(): void {
    ...
    const ids = [...this.editor.selectedItemIds];
