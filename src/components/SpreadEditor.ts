...
export class SpreadEditor {
   ...
   */
  private createItemNode(item: PageItem, xOffset: number, pageNumber: number): Konva.Shape | Konva.Text | Konva.Arrow | null {
    ...
    if (node) {
      ...
      if (item.type === 'text') {
        node.on('dblclick dbltap', () => {
          this.startTextEditing(node as Konva.Text, item as TextPageItem, pageNumber, xOffset);
        });
      }
   ...
   */
  private startTextEditing(textNode: Konva.Text, item: TextPageItem, pageNumber: number, xOffset: number): void {
    ...
    this.stage.container().parentElement!.appendChild(textarea);
    ...
    textarea.style.width = `${textNode.width() * this.zoomLevel}px`;
    textarea.style.minHeight = `${textNode.height() * this.zoomLevel}px`;
    textarea.style.fontSize = `${item.fontSize * this.zoomLevel}px`;
